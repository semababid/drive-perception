...
phases. Each numbered item is one commit. Milestone checkpoints mark the points where
the project is already worth showing even if the rest is unfinished.

Detector backbone is **YOLO11**. Two sizes run through the whole pipeline: `yolo11n`
is the fast edge/deploy target, `yolo11s` is the accuracy anchor. The benchmark
reports both so the results read as an accuracy-vs-latency curve, not a single point.

## Phase A — Foundation
1. `chore: git init, .gitignore, MIT license`
2. `chore: add pyproject and dependencies`
3. `chore: tooling — ruff, pytest config`
4. `chore: folder skeleton and config loader`
5. `ci: lint and smoke-test workflow`

## Phase B — Data
6. `feat: KITTI download script with subset flag`
7. `docs: dataset EDA notebook`
